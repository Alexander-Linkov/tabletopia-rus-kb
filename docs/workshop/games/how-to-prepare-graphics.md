...
Убедитесь, что у всех компоненты одного типа в точности совпадают размеры, вплоть до пикселя. Это важно, если вы будете использовть колоды\стопки или [Случайные элементы](../enhance/randomizers.md). Разница даже в пиксель - критична.

> **Совет:** В статье [Как улучшить визуальный стиль игры](../enhance/visual-design-guide.md) есть предложения по улучшению грифики игры и ее представления в Tabletopia.
...
Иногда использование пропорционых размеров больших компонентов (например [игровых полей](../objects/game-board.md)) приводит к увеличению размеров файла сетапа и сильное потребление ресурсов компьютера.
...
* Разделите игра на несколько сетапов, для различного количества игроков, или с различным набором компонентов. Подробнее читайте в статье [Сетапы игры](game-setups.md)
